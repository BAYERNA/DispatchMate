...
try {
  ...
  await db.exec(await readFile(new URL('V15__communication_health_monitoring.sql', migrations), 'utf8'));
  await db.exec(await readFile(new URL('V16__command_receipts_and_communication_recovery.sql', migrations), 'utf8'));
  ...
  await mission.commandReceipt(command.commandId,{userId:a,badgeNumber:'0011',role:'RESPONDER'},'RECEIVED');
  await mission.commandReceipt(command.commandId,{userId:a,badgeNumber:'0011',role:'RESPONDER'},'READ');
  await mission.commandReceipt(command.commandId,{userId:a,badgeNumber:'0011',role:'RESPONDER'},'ACCEPTED');
  await assert.rejects(()=>mission.commandReceipt(command.commandId,{userId:b,badgeNumber:'0012',role:'RESPONDER'},'READ'));
  assert.equal((await rows(`SELECT status FROM incident_commands WHERE command_id=$1`,[command.commandId]))[0].status,'ACKNOWLEDGED');checks++;
  ...
  const responderControl=await mission.control(incident,{userId:a,badgeNumber:'0011',role:'RESPONDER'});assert.equal(responderControl.packets.length,0);assert.equal(responderControl.channelAttempts.length,0);assert.ok(responderControl.commandReceipts.every(item=>item.userId===a));checks++;
  ...
  const heartbeat=await advanced.communicationHeartbeat(incident,responderUser,{clientId:'web-test-client',state:'CONNECTED',transport:'CELLULAR',socketConnected:true,clientOnline:true,downlinkMbps:12.5,rttMs:80});assert.equal(heartbeat.effectiveState,'CONNECTED');
  const communicationDashboard=await advanced.dashboard(incident,commanderUser);assert.equal(communicationDashboard.communications.find(item=>item.userId===a).effectiveState,'CONNECTED');
  await assert.rejects(()=>advanced.communicationHeartbeat(incident,responderUser,{clientId:'web-test-client',state:'SAFE',transport:'UNKNOWN'}));checks++;
  ...
  await rows(`UPDATE incident_communication_heartbeats SET last_heartbeat_at=now()-interval '1 minute' WHERE incident_id=$1 AND user_id=$2`,[incident,a]);
  await rows(`UPDATE incident_assignments SET assigned_at=now()-interval '1 minute' WHERE incident_id=$1 AND user_id=$2`,[incident,b]);
  await automation.communicationSafety();assert.ok(automated.some(item=>item.automationKey.startsWith('communication-stale:')));assert.ok(automated.some(item=>item.automationKey.startsWith('communication-never:')));checks++;
  const recovering=await advanced.communicationHeartbeat(incident,responderUser,{clientId:'web-test-client',state:'CONNECTED',transport:'CELLULAR',socketConnected:true,clientOnline:true});assert.equal(recovering.effectiveState,'RECOVERING');
  const recovered=await advanced.communicationHeartbeat(incident,responderUser,{clientId:'web-test-client',state:'CONNECTED',transport:'CELLULAR',socketConnected:true,clientOnline:true});assert.equal(recovered.effectiveState,'CONNECTED');assert.ok(alertsCreated.some(item=>item.automationKey.startsWith('communication-recovered:')));checks++;
  const overdue=await mission.createCommand(incident,commanderUser,{commandType:'MUSTER',assigneeId:b,title:'즉시 인원 확인',acknowledgementSeconds:30});await rows(`UPDATE incident_commands SET acknowledgement_due_at=now()-interval '1 second' WHERE command_id=$1`,[overdue.commandId]);await automation.commandAcknowledgementSafety();assert.ok(automated.some(item=>item.automationKey===`command-unacknowledged:${overdue.commandId}:${b}`));checks++;
